# HTML

## Metadata

- [The Open Graph protocol](https://ogp.me)
- [About Twitter Cards](https://developer.twitter.com/en/docs/twitter-for-websites/cards/overview/abouts-cards)
- [The Essential Meta Tags for Social Media | CSS-Tricks](https://css-tricks.com/essential-meta-tags-social-media/)
- [<meta>: The metadata element  | MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta)
- [Standard metadata names | MDN](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta/name)
